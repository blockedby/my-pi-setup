...
- `feature-pipeline`: controller-owned five-track discovery and synthesis, three isolated parallel Luna/xHIGH implementation checkpoints, one Luna/xHIGH read-only selection plus attributable augmentation and production completion, exact promotion/cleanup, a Luna/xHIGH post-promotion remediation root, four Luna audits, reusable five-contributor final Luna audit segment, root final resolution and factual completion;
...
Controller-owned five Luna/medium discovery tracks in parallel
  ...
  → three committed Shape → Prove → Checkpoint handoffs and frozen refs
  → one Luna/xHIGH selection-only comparison
  → exactly one validated primary checkpoint
  → separate synthesis worktree from the exact primary commit
  → optional bounded attributable augmentation
  → production completion, full relevant verification, and final synthesis commit
...
Each works only in its controller-owned worktree and stops at a viable implementation checkpoint rather than completing the feature for merge. Shape materializes the distinguishing contracts, boundaries, control/data flow, integration points, and trade-offs in real code. Prove makes the main path work end-to-end through its critical integration and exercises the most important invariant, edge/failure case, or architectural assumption. Checkpoint runs focused repository-native verification and fixes only defects that undermine viability. A candidate continues while its central flow is disconnected, its critical risk remains an assumption, or completion would require replacing the core design; it stops once the remaining work is production completion such as secondary behavior, tests, plumbing, cleanup, documentation, or broad regression verification.

Every candidate leaves a clean branch with at least one ordinary implementation commit and returns the strict `feature-implementation-candidate-v2` handoff: role, approach summary, changed paths, proven behavior, checks, remaining work, trade-offs, assumptions, and worktree/branch/base/head identity. Remaining work describes acceptance, behavior, verification, plumbing, cleanup, or documentation gaps analytically rather than prescribing line edits. Host validation compares every Git/path field and changed-path list to actual committed state and proves each head descends from the captured base. A changed historical test is allowed when the behavior change is justified. Candidate built-in read/edit/write/bash tools are replaced by controller-scoped definitions: filesystem operations resolve only beneath the assigned worktree, and bash runs inside bubblewrap with the host root read-only, the run temporary root masked, only the assigned worktree rebound writable, network namespaces isolated, and shared Git metadata masked. Agents request ordinary commits through `pipeline_feature_commit`; only the controller stages/commits the role-owned worktree. Settled candidates are frozen by controller state: neither root continuation/cancellation APIs nor agent-tree view send/cancel controls can target them, while whole-run controller cancellation remains available; the controller rechecks clean immutable heads before selection. Full transcripts and tool history are not synthesis inputs.

One persistent Luna/xHIGH `implementation-synthesis` session owns two distinct turns. Its first turn runs in an empty controller-owned `selection` directory with no synthesis worktree or synthesis branch. It receives the common package plus each compact handoff, changed paths, immutable commit/worktree reference, checks/trade-offs, and a per-candidate diff bounded at 48 KiB; total synthesis input is bounded at 512 KiB. It compares in this exact order: correctness, acceptance coverage, regression risk, repository fit, simplicity, maintainability, verification quality. Candidates are intentionally incomplete: selection judges each materialized approach, its proven behavior, and whether its remaining production work is understandable and safe. Expected checkpoint incompleteness is not itself a negative, while uncertain or architectural remaining work remains a real risk. It chooses the simplest viable foundation with the best evidence for a complete reliable solution, never role name, raw size, apparent completeness, or ambition. The host requires all roles exactly once, exactly one viable primary checkpoint, verifies that the selection directory remained empty and all candidates remained frozen, and fails explicitly if none is viable. During selection only scoped read and bubblewrap read-only bash are active. After selection the controller atomically enables the same session's scoped edit/write and commit tools for its primary-based synthesis worktree; frozen candidate worktrees remain visible as read-only sources while only synthesis is writable.

Only after valid selection does the controller replace that empty selection path with a linked synthesis worktree created from the exact primary commit; the pre-validation directory is never named or registered as synthesis Git state. The same Luna session receives its second turn and owns two write stages.

First it may accept zero or a small number of objectively beneficial losing-candidate ideas: simpler local code, concrete edge-case/test/invariant handling, a better boundary, or a justified small structural improvement. It does not combine all approaches mechanically. Every selection idea names exact unique source paths from a losing candidate's committed diff. Every accepted augmentation maps each source path exactly once to a unique path whose blob or deletion at the distinct augmentation commit exactly matches the frozen source. This preserves mechanically verifiable provenance for borrowed whole-file material. The primary-to-augmentation change remains capped at 64 paths and 128 KiB; an empty augmentation commit records that no borrowed idea was useful.

The synthesis agent then completes the selected approach to production readiness. It uses the primary handoff's remaining work and the discovery contract as analytical inputs, closes all required acceptance behavior and necessary edge/failure cases, finishes plumbing, tests, feature-related cleanup and relevant documentation, and performs the final repository verification required by the feature. Completion changes may author or revise any in-scope file, including an augmented file, and are not required to match a losing candidate. A distinct final commit must descend from the augmentation commit, the worktree must be clean, and reported completion paths must match the augmentation-to-final diff. Every reported synthesis check must exactly match a successful command observed by the controller in the synthesis sandbox after primary selection; unobserved and failed-command claims are rejected before promotion. The host does not claim to semantically classify arbitrary repository-specific commands or prove architectural similarity from Git bytes alone: preservation of the selected approach is an explicit synthesis contract and the independent post-promotion audit checks the completed result. This separates strict borrowed-idea provenance from the synthesis agent's responsibility to finish the implementation.
