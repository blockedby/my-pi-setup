...
The controller then creates a separate Luna/xHIGH post-promotion root in the original caller worktree. Candidate roles, winner identity, rationale, borrowed ideas, temporary paths/refs, and commits remain internal. Pre-final and final audit contexts are host-built from the original task, validated discovery reports/synthesis, acceptance/constraints/invariants, sanitized verification checks, and fresh promoted/remediated Git evidence; root-supplied provenance-bearing context is ignored. Existing four-track audit, remediation, reusable final audit, final resolution, and factual completion behavior continues against only the caller worktree. No Sol or direct-subagent quota is used in the candidate/selection/augmentation segment, no discovery is repeated, and no audit-based model-profile escalation exists.
...
`pipeline_complete` continues to emit facts rather than readiness. Standalone audit completion is controller-owned after strict final validation and includes the bounded structured final audit report. Feature and plan roots still call `pipeline_complete` after their own final resolution; feature completion augments the Luna remediation root's report with fresh final host Git observations. The calling main agent owns readiness, remediation outside the standalone audit, and all branch/commit/push/PR decisions beyond the narrowly opted-in ordinary commits described above.
