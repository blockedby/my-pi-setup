...
- **Action:** Merged [PR #11](https://github.com/blockedby/my-pi-setup/pull/11) for context occupancy correctness, rebased the independent advisory branch onto the resulting `main`, resolved only the combined deterministic test script and operation-record conflicts, reran integration checks, then merged [PR #12](https://github.com/blockedby/my-pi-setup/pull/12). Reinstalled the repository into Pipi with `npm run install:pipi -- --skip-dependencies`; the installed runtime remains `0.83.0`. Local feature worktrees and branches were removed; remote feature branches were retained by policy.
