...
- **Verification:** The updated subagents deterministic suite passed 28/28; file-search passed 22/22; TypeScript, formatting, submodule, installer, and `git diff --check` checks passed. The canonical initial review identified omitted Pi `grep.details.linesTruncated` handling; the classifier and retained regression test were corrected and the same checks passed again. No root/live Claude/Codex tests were run.
- **Pending:** Complete the canonical closure review, open the PR to `main`, merge only after authorization, reinstall/reload Pipi after merge, and verify advisory injection with deterministic/runtime-safe evidence.
