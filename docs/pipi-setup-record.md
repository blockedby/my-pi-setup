...
## Operation entry: speed-first parallel Luna delegation

- **Request:** Make the main agent use more Luna agents in parallel to improve speed, improve swarm orchestration, research real-world examples, then open a pull request for review.
- **Action:** Changed model-facing and subagent-skill guidance so nontrivial work with two or more dependency-ready, independent scopes defaults to one Luna profile per scope in the same parallel wave before broad sequential Sol exploration or planning; routine independent work must select `luna-explore` or `luna-worker`, not bare Pi/inherited Sol. Added a lightweight wave manifest (scope/id, prerequisites, exclusive edit scope, acceptance check), scoped conclusion-first reports, automatic-result dependency gates, narrow retry rules, and conditional Terra audits. Kept local-work exceptions for trivial, dependent, overlapping-write, or latency-negative work. Removed the arbitrary eight-agent wording; the existing 16-Luna direct quota remains capacity, not a target. Updated the subagent design plan and added deterministic prompt-contract coverage.
- **Evidence:** Fetched OpenAI's [Multi-agent guide](https://developers.openai.com/api/docs/guides/responses-multi-agent), Anthropic's [multi-agent Research article](https://www.anthropic.com/engineering/multi-agent-research-system), and the [Strands Graph pattern](https://strandsagents.com/docs/user-guide/concepts/multi-agent/graph/). These primary sources support bounded independent tasks, orchestrator-owned synthesis, explicit worker objectives/outputs/boundaries, dependency-driven fan-out/fan-in, and conditional scale-up. A disposable browser-control session was acquired, but the headless DevTools MCP server timed out; the HTTP-fetched evidence remains available.
- **Affected paths or values:** `extensions/subagents/src/prompt.ts`, `extensions/subagents/prompt-contract.test.ts`, `extensions/subagents/docs/design-plan.md`, `skills/subagents/SKILL.md`, and this record. No runtime configuration, model overrides, or quota changed.
...
- **Action:** Added the package prompt template `prompts/solve-issue.md`, invoked as `/solve-issue <issue URL or description>`. It directs the main agent to run an inline workflow with three concurrent read-only Luna discovery agents; a Sol planning gate; up to four disjoint Luna implementation agents running concurrently; Sol integration; a readiness sweep that launches newly unblocked independent leaf tasks within the original issue scope; a conditional Terra audit for high-risk, cross-cutting, or disputed work; then Sol verification. It explicitly prevents commits, pushes, closing issues, and external-state changes. Added README discovery documentation.
...
- **Verification:** Parsed the embedded workflow through `prepareWorkflowScript` and ran it in the permission-restricted workflow sandbox with deterministic child results. The behavioral tests prove the tool's JSON-string argument contract; three concurrent discovery agents; a plan gate; two concurrently dispatched implementation tasks; a readiness sweep that dispatches a newly unblocked parallel leaf wave; conditional Terra audit; and that lexical or path-alias ownership overlap blocks implementation before any writer is launched. They do not assert prompt wording. Passed `npm run check`, `npm run test:extensions` (147 deterministic tests), `npm run format:check`, and `git diff --check`.
- **Closure review and pull request:** The initial review found two blockers: object-valued workflow arguments and plan-path overlap. The first closure review confirmed the argument boundary fixed and found one remaining repeated-separator alias bypass; the final closure review of remediation commit `f9fbf95` returned `READY`, confirming normalized/rejected aliases block before implementation dispatch. Pushed the completed workflow updates to [PR #20](https://github.com/blockedby/my-pi-setup/pull/20).
- **Pending:** Review the workflow design in PR #20; no live workflow or model invocation has been run.

## Operation entry: main-chat fallback for nonparallel work

- **Request:** Make main-chat execution mandatory when work cannot be genuinely parallelized, because serial subagents add avoidable latency.
- **Action:** Updated direct-subagent, workflow, and `solve-issue` guidance: use a parallel wave only for independent, dependency-ready scopes; keep ordered, dependency-heavy, or shared-mutable-ownership work in the main chat instead of serializing it through agents.
- **Affected paths or values:** `extensions/subagents/src/prompt.ts`, `skills/subagents/SKILL.md`, `extensions/workflows/prompt.ts`, `prompts/solve-issue.md`, and this record. Runtime behavior, models, and quotas are unchanged.
- **Verification:** Passed the focused workflow-template behavior test and full `npm run check`, `npm run test:extensions` (147 deterministic tests), `npm run format:check`, and `git diff --check` in the feature worktree.
- **Pending:** Review the new routing rule and workflow design in PR #20; no live workflow or model invocation has been run.
