...
- **Verification:** Submodule validation accepted both exact clean gitlinks. Installer tests passed before remediation, and the full deterministic suite passed 28 script/installer tests, 144 extension tests, and 22 file-search tests. TypeScript and formatting passed after installing worktree dependencies. The pinned child passed 13 Python unit tests; Pi's skill loader discovered exactly one `plan-gh-backlog` skill; and the bundled CLI validated and planned its complete example. Final post-remediation and closure-review evidence will be recorded before publication.
- **Pending:** Complete post-remediation checks and canonical closure review, open the PR to `main`, install a filtered local package entry for the skill without duplicating existing Pipi resources, verify discovery, and reload the active Pipi session if needed. Merge remains a separate decision.
