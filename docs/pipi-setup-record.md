...
## Operation entry: design model-facing pipeline inspection tools

- **Request:** Design a model-facing `pipeline_status` capability and determine whether direct subagents already have an equivalent. After repository inspection, the user chose to preserve the existing subagent semantics and names as `pipeline_check` plus `pipeline_list` rather than introduce one combined status tool.
- **Evidence:** Direct subagents expose nonblocking `subagent_check({ id })` with bounded recent activity and `subagent_list({})` with all model-visible snapshots. Pipelines currently expose only `pipeline_run` to the main model and `/pipelines` to the interactive user, although `PipelineController` already has newest-first `list()` and ID-based `get()` snapshots.
- **Design action:** Added the planned parent-inspection contract to `docs/pipelines-v1-design.md`: strict `{ id }` and `{}` schemas; nonblocking/read-only/non-consuming behavior; compact run, stage-progress, elapsed, agent-count, and role/attempt/status output; explicit structured projections that exclude prompts, transcripts, thinking, final output, session files, tool names, raw reports, Git evidence, and unbounded completion facts; clear unknown-ID/empty behavior; main-agent-only tool policy; and anti-polling guidance with no `pipeline_wait`.
- **Affected paths or values:** `docs/pipelines-v1-design.md`, this operation record, and design branch/worktree `design/pipeline-inspection-tools`. Runtime code, installed Pipi, current pipelines/subagents, dependencies, model overrides, credentials, and submodule pins are unchanged.
- **Verification:** Cross-checked the design against `extensions/subagents/index.ts`, `extensions/subagents/src/prompt.ts`, `extensions/pipelines/index.ts`, `extensions/pipelines/controller.ts`, pipeline domain snapshots, and shared agent-tree snapshot fields. Reviewed the rendered section and `git diff --check` passes.
- **Pull request:** Committed as `5971e94`, pushed `design/pipeline-inspection-tools`, and opened [PR #40](https://github.com/blockedby/my-pi-setup/pull/40) against current `main`.
- **Pending:** Review the design PR. Implementation, runtime tests, merge, installation, and reload require a later explicit request.
