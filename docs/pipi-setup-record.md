...
- **Action:** Synchronized merged PR #16 into the primary `main` checkout, refreshed Pipi from that canonical package, removed the retired filtered package entry and old feature worktree, then created `.worktrees/update-plan-gh-backlog` on `chore/update-plan-gh-backlog`. Fetched `origin/main` inside the read-only child and provisionally advanced the parent gitlink from `29136202437149b477e5d21317d82219fcc011bb` to commit `bbdf31286597daa440ab70efe207378d4bbc74d2`, which adds sanitized apply-progress and retry logging plus network-timeout retries. Canonical review found that this upstream commit regressed removal of existing milestone due dates, so the blocked worktree package was removed from live Pipi and the canonical primary package at `2913620` was restored. Implemented the upstream fix in a separate `plan-gh-backlog` worktree and opened [child PR #2](https://github.com/blockedby/plan-gh-backlog/pull/2) to `main`.
- **Affected paths or values:** `vendor/plan-gh-backlog` and this record. The final safe child pin is `5a179fb453c6b97ce5c93723319e691dac27bc18`; the child still reports CLI version `1.0.0`. Pipi runtime, MCP version, model overrides, credentials, auth isolation, profiles, and quotas are unchanged.
- **Verification:** The provisional child passed all 15 standard-library Python tests and its bundled CLI validated and planned the complete example. Canonical review returned `NOT_READY` with `REV-001` for the due-date clearing regression. The focused child fix passes 16/16 tests and explicitly proves one-time `due_on: null` reconciliation followed by an idempotent rerun. Authorized child PR #2 was squash-merged to `main` as `5a179fb`; the parent validates both exact clean gitlinks at that final pin and passes all 29 installer/script tests, TypeScript, formatting, and `git diff --check`. Canonical closure marked `REV-001` fixed and returned `READY` with no new findings. The Pipi installer completed from the update worktree; package resolution exposes exactly its one `plan-gh-backlog` skill, disables the old primary copy, and loads no other update-worktree resources. `pipi --version` remains `0.84.2`, and the latest bundled CLI reports `1.0.0`.
- **Pending:** Reload or restart the active Pipi session. [Parent PR #17](https://github.com/blockedby/my-pi-setup/pull/17) is open to `main`; merge remains a separate decision. After merge, rerun the primary installer and remove the temporary filtered worktree package before cleanup.
