...
- **Pending:** Reload or restart sessions created before this rollout so their in-memory extension registry exposes `pipeline_check` and `pipeline_list`; newly started sessions load the merged tools. Design PR #40 remains open and separate. No further repository or installer action is pending.
