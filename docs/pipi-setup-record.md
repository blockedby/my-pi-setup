...
- **Review:** Canonical initial review returned `NOT_READY` with `REV-001` (pipeline capacity did not share direct-subagent admission) and `REV-002` (`x` on an idle root did not cancel the active run). The user explicitly accepted independent pipeline capacity as intended product behavior, so the design now records pipeline-local Sol=4, Terra=8, and Luna=16 pools rather than a shared quota requirement. `REV-002` remediation routes root/run cancellation through the pipeline controller, marks an idle persistent root cancelled, cancels active children, and emits one handoff; deterministic regression coverage exercises that UI/controller path.
- **Pending:** Complete closure review for `REV-002` and confirm `REV-001` is not applicable under the clarified requirement. Runtime acceptance against real Sol/Luna/Terra sessions remains intentionally unrun until explicitly authorized.
