...
- **Verification:** The provisional child passed all 15 standard-library Python tests and its bundled CLI validated and planned the complete example. Canonical review returned `NOT_READY` with `REV-001` for the due-date clearing regression. The focused child fix passes 16/16 tests and explicitly proves one-time `due_on: null` reconciliation followed by an idempotent rerun. Authorized child PR #2 was squash-merged to `main` as `5a179fb`; the parent now validates both exact clean gitlinks at that final pin and passes all 29 installer/script tests, TypeScript, formatting, and `git diff --check`. Until final installation, Pipi package resolution continues to expose only the safe primary `2913620` skill.
- **Pending:** Complete closure review, install the safe latest pin from the filtered update worktree, open the parent PR to `main`, and reload or restart the active Pipi session afterward. Parent merge remains a separate decision.
