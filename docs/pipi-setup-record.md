...
- **Affected paths or values:** `extensions/subagents/src/prompt.ts`, `extensions/subagents/spawn-contract.test.ts`, and this record. Direct quotas remain Sol=4, Terra=8, Luna=16; the workflow cap and soft Luna fanout recommendation remain eight.
- **Verification:** Added a focused prompt-contract test covering one bounded deliverable, explicit edit ownership, separate concerns, pseudocode, the feature example, same-wave eight-worker fanout, non-serialization, and main-agent conflict/result integration. The first deterministic run exposed an existing prompt contract requiring the exact `cross-cutting integration and final acceptance` boundary; the new detailed integration guideline now preserves that phrase rather than weakening the established contract. The corrected full run passed 29 script/installer/submodule tests, 145 deterministic extension tests, and 22 file-search tests; TypeScript, formatting, submodule validation, Pipi version/install checks, and `git diff --check` passed.
- **Review:** Canonical initial review returned `READY` with no findings and independently confirmed the focused contract plus the complete deterministic/check matrix.
- **Pull request:** Committed the isolated change as `a812c67` (`feat(subagents): guide parallel Luna decomposition`), pushed `feat/luna-delegation-scoping-guidance`, and opened [PR #18](https://github.com/blockedby/my-pi-setup/pull/18) against `blockedby/my-pi-setup` `main`.
- **Preparation:** Target-branch preparation fetched `origin/main` and reported `rebase_status=up_to_date`, no conflict, `content_changed=false`, and `rerun_required=false`; the reviewed and verified product diff did not change.
- **Pending:** Push this final record, review/merge PR #18, and reload or restart Pipi after merge.
