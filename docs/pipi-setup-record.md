...
- **Verification:** The provisional child passed all 15 standard-library Python tests and its bundled CLI validated and planned the complete example. Parent checks passed for both exact clean gitlinks, all 29 installer/script tests, TypeScript, formatting, and `git diff --check`. Canonical review returned `NOT_READY` with `REV-001` for the due-date clearing regression. The focused child fix now passes 16/16 tests, explicitly proves one-time `due_on: null` reconciliation followed by an idempotent rerun, and child PR #2 is open and mergeable. Pipi package resolution confirms that only the safe primary `2913620` skill is currently active.
- **Pending:** Merge child PR #2, fetch its resulting latest `main` commit, advance and re-verify the parent gitlink, complete closure review, install the safe latest pin, and open the parent PR to `main`. Reload or restart the active Pipi session afterward.
