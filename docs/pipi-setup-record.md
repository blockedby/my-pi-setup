...
- **Pending:** If approved, implement a dashboard-only effective-stage projection at successful fan-in boundaries, rename the active stage display to `running`, move the orange active indicator to that stage row, retain child result glyphs, and add focused row-model/render-state regression tests before running the repository checks.
