...
4. Run fresh bounded validation with pipeline_plan_validate and capture Git state with pipeline_git_status. Mark audit. Launch exactly these four Luna/medium roles in one parallel wave: audit-product-traceability, audit-decomposition-dag, audit-cross-layer-integration, audit-test-release-reliability. Give each the goal, repository evidence, assumptions, plan path/content, and validation evidence. Wait for every report.
5. Mark audit-resolve. Resolve every actionable Luna finding in the plan once, or reject it with specific evidence. Revalidate the plan.
6. Mark final-audit. Launch one independent Terra/high final-audit child. Give it the original goal, synthesized repository evidence, assumptions, current plan path/content, and fresh validation evidence. Do not include Luna findings or their resolutions. Terra must read and follow the canonical code-review skill in initial mode, adapted from implementation defects to concrete plan-quality defects. Wait for its report; do not retry this role.
7. Mark final-resolve. Resolve Terra's actionable findings in the plan once, or reject them with evidence. Revalidate the artifact. Do not audit again.
