...
export function buildFeaturePipelinePrompt(
  ...
  return `You are the persistent Astra/low audit and remediation root for one feature-pipeline run. The supplied linked worktree is the final reviewed implementation workspace. Audit and remediate this implementation against the supplied canonical requirements and current evidence. Do not implement a new solution or repeat discovery or planning.
...
export function buildFeatureExecutionGraphPrompt(
  ...
  return `Compile the accepted immutable canonical plan below into a small-task series-parallel execution graph for fresh Astra/low implementers. Preserve the plan; do not reopen architecture selection.

Accepted canonical plan:
${JSON.stringify(canonicalPlan)}

${FEATURE_PLANNING_READINESS_INSTRUCTION}
Every task must be one coherent, independently verifiable unit with enough goal, repository context, conventions, precedents, invariants, implementation instructions, done conditions, acceptanceRefs, and checks for its Astra/low implementer. A task may produce an append-only checkpoint range, or no commits when already satisfied; do not require one synthetic commit. Preserve the series-parallel DAG, dependencies, fork/join structure, and acceptance coverage without artificial fork or join tasks. The controller owns branch integration and join repair.

Baseline checks run before branch implementation; readiness, preparation, and baseline failures are diagnostic recovery context, not proof that implementation cannot start. Declare executable verification separately from preparation and manual acceptance. Checks run offline in the assigned sandbox without live Git-metadata access; git diff --check is controller-mediated against the task base. Caller-owned worktree_prepare remains initial preparation; task-local environment repair uses explicit pipeline_task_prepare, whose authorized network access is not limited to package registries. Neither preparation nor checks authorize push, deploy, external delivery mutation, or raw Git bypass.

${FEATURE_TASK_LIFECYCLE_INSTRUCTION}

Each task retains its persistent implementation session for routine recovery. Use concrete diagnostics to repair preparation, code, check recipes, or acceptance handoff; do not replace the worker for ordinary failures. Recovery has a bounded no-progress budget: report unresolved blockers and uncertainty when exhausted, rather than looping or claiming success. Remaining-graph replanning is not available; do not promise it or submit a replacement graph. Remain read-only as graph compiler. Submit exactly one feature-execution-graph-v1 object through pipeline_feature_execution_graph_submit and stop. If rejected, correct the exact graph validation errors in this same session.${submissionExample(FEATURE_EXECUTION_GRAPH_EXAMPLE_ROLE)}`;
}
...
Compare the complete implementation with the canonical acceptance criteria and current code. Inspect diagnostics, including readiness, preparation, baseline, and join failures, as recovery context; repair missing behavior or weak integration directly in this same session.

${FEATURE_TASK_LIFECYCLE_INSTRUCTION}

Use pipeline_task_prepare for necessary explicit environment repair; sandbox-authorized preparation network access is not registry-only. Do not push, deploy, mutate external delivery state, or bypass controller tools with raw Git. Preserve actual results, unresolved issues, and uncertainty in the detailed final summary. Do not create another task graph, promise remaining-graph replanning, delegate, or broaden the accepted scope. If bounded recovery makes no progress, report the concrete blocker instead of claiming acceptance.`;
}
...
const FEATURE_CHECK_CWD_INSTRUCTION =
  'Every check cwd is relative to the assigned worktree root: use "." for its root or "apps/core" for a package directory. Never put an absolute filesystem path, the displayed working_dir/workspaceRoot, a leading slash, "~", or a ".." component in cwd. The controller resolves cwd inside each assigned worktree, including child branches. Record observed command/cwd pairs faithfully; do not replace "." with working_dir. Before submitting, check every verification, baselineChecks, task check, and review check cwd against this rule.';

const FEATURE_READINESS_DISCOVERY_INSTRUCTION = `Inspect applicable AGENTS.md/repository instructions, manifests/scripts, and CI with read/fd/rg. Select appropriate noninteractive verification commands and request each through pipeline_feature_readiness_check with {command,cwd,purpose,source:{path,excerpt}}. ${FEATURE_CHECK_CWD_INSTRUCTION} Prefer repository-native runner entrypoints so local binaries and runtime configuration remain available. Cite the actual repository-relative source path and copied excerpt when available; never fabricate provenance. Source confirmation is diagnostic provenance, not an exact-invocation execution gate or a grant of permission: uncertain or missing provenance must stay explicit, while the sandbox independently authorizes execution. Preserve the returned command, cwd, purpose, provenance, actual status/output, and uncertainty. Failed, denied, unavailable, or contradictory checks are recovery context, not passing evidence. Discovery does not install/bootstrap, use network for verification, or mutate Git. Only this role requests readiness checks; other discovery roles do not duplicate them.`;

const FEATURE_PLANNING_READINESS_INSTRUCTION = `${FEATURE_CHECK_CWD_INSTRUCTION} The parent appends controller-observed diagnostic results. Preserve their provenance, actual outcomes, and uncertainty rather than turning source confirmation into a readiness gate or failures into success. Carry original verification obligations and acceptance coverage into candidate, canonical, and graph plans. Commands are initial recipes, not immutable proof: evidence-backed task-time adjustments must retain those obligations and record their reason. Distinguish existing baseline checks from future checks whose scripts/tests must first be implemented; do not put future scripts in graph baselineChecks. Keep preparation separate from verification and missing or contradictory evidence explicit.`;

const FEATURE_TASK_LIFECYCLE_INSTRUCTION = `Preferred implementation lifecycle:
1. Inspect pipeline_task_diff and supplied diagnostics. When environment repair is needed, explicitly call pipeline_task_prepare with command, repository-relative cwd, and purpose; inspect its actual result.
2. Edit only the assigned workspace. Select exact repository-relative paths through pipeline_task_stage with action stage or unstage; record intentional progress through pipeline_task_checkpoint with a meaningful message. Checkpoints append commits without amending or rewriting earlier progress. Inspect residual paths and resolve them explicitly without discarding unrelated work.
3. Run pipeline_task_check for required check IDs. To adjust a recipe, supply checkId, recipe {command,cwd,purpose,reason}, and acceptanceRefs grounded in the task. Original required checks and acceptance obligations cannot disappear or be weakened to manufacture success. Supplementary checks are permitted with a new checkId, complete recipe and explicit acceptanceRefs; they supplement rather than replace original obligations.
4. Evidence belongs to the exact checked output content and recipe. A passing build that generates outputs may be accepted after checkpointing that exact content; do not demand a rerun-to-fixed-point merely because it generated files or HEAD advanced. Subsequent content edits or recipe changes invalidate affected evidence; run the necessary checks on the new state. Preserve actual failed results and uncertainty in the handoff.
5. Call pipeline_task_finalize with summary only once all required checks have current passing evidence and the meaningful workspace is clean. Finalize verifies existing evidence and the controller-owned checkpoint range; it does not implicitly stage, discard, or run all checks. With no changes, use the same summary-only acceptance. If rejected, repair the reported issue in this session and retry within the bounded recovery budget.`;
