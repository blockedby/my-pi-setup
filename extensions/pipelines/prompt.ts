...
const FEATURE_CHECK_CWD_INSTRUCTION =
  'Every check cwd is relative to the assigned worktree root: use "." for its root or "apps/core" for a package directory. Never put an absolute filesystem path, the displayed working_dir/workspaceRoot, a leading slash, "~", or a ".." component in cwd. The controller resolves cwd inside each assigned worktree, including child branches. Preserve controller-verified command/cwd pairs exactly; do not replace "." with working_dir. Before submitting, check every verification, baselineChecks, task check, and review check cwd against this rule.';

const FEATURE_READINESS_DISCOVERY_INSTRUCTION = `Inspect applicable AGENTS.md/repository instructions, manifests/scripts, and CI with read/fd/rg. Choose only appropriate existing noninteractive verification commands. Before submission, run each via pipeline_feature_readiness_check with {command,cwd,purpose,source:{path,excerpt}}. ${FEATURE_CHECK_CWD_INSTRUCTION} Use the repository's normal verification entrypoint and preserve its runtime environment: for example bun run lint, pnpm run check, npm run test, or yarn run test, selected from repository instructions and configuration rather than guessed. Do not extract a package script body such as oxlint for direct execution instead of its runner; local binaries may then be missing from PATH. source.path is repository-relative and the real file must literally contain the copied excerpt. A bare bun/npm/pnpm/yarn run <script> invocation (without extra arguments or shell chaining) can cite the script's key and string value in the package.json belonging to cwd; the invocation itself need not appear in that excerpt. Other commands and wrappers, including Composer or Sail, require the exact invocation in repository instructions or CI. These examples do not authorize installing tools or inventing wrappers. JSON escapes in package script bodies are decoded without changing the command. Results are authoritative evidence. No install/bootstrap/network/Git mutation or fabricated readiness. Report missing or contradictory commands; never invent replacements. Only this role runs these checks; other discovery roles do not duplicate them.`;

const FEATURE_PLANNING_READINESS_INSTRUCTION = `${FEATURE_CHECK_CWD_INSTRUCTION} The parent will append the controller result handoff; treat it as authoritative. Candidate, canonical, and graph plans must carry source-confirmed existing commands as baseline or pre-existing checks, preserving exact command/cwd/purpose and source/result. Do not rerun, wrap, invent, or replace checks; keep missing or contradictory ones explicit. A future script/test must be created by a task before it is checked and never belongs in graph baselineChecks.`;
